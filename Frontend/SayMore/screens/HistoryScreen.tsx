...
const HistoryScreen: React.FC<HistoryScreenProps> = () => {
    ...
    const fetchVideoDetails = useCallback(async (videoId: string, lessonTitle: string): Promise<any> => {
        try {
    ...
    const handleVideoPress = useCallback(async (video: WatchedVideo) => {
        // Show loading indicator
        setLoading(true);

        try {
            // Fetch complete video details
            const videoDetails = await fetchVideoDetails(video.videoId, video.lessonTitle);
    ...
    const renderItem = useCallback(({ item }: { item: WatchedVideo }) => (
        <TouchableOpacity style={styles.historyItem} onPress={() => handleVideoPress(item)}>
            <Image
                source={{ uri: item.thumbnail || 'https://via.placeholder.com/80x60' }}
    ...
    return (
        <View style={styles.container}>
            ...
            {loading ? (
                ...
                <FlatList
                    data={watchedVideos}
                    keyExtractor={(item) => item.id || `${item.videoId}-${item.timestamp}`}
